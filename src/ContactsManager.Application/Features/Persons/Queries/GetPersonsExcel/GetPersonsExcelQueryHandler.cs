...
public sealed class GetPersonsExcelQueryHandler(
    IAppDbContext context,
    ILogger<GetPersonsExcelQueryHandler> logger,
    IPersonExportService personExportService
) : IRequestHandler<GetPersonsExcelQuery, Result<byte[]>>
{
    private readonly IAppDbContext _context = context;
    private readonly ILogger<GetPersonsExcelQueryHandler> _logger = logger;
    private readonly IPersonExportService _personExportService = personExportService;

    public async Task<Result<byte[]>> Handle(
        GetPersonsExcelQuery request,
