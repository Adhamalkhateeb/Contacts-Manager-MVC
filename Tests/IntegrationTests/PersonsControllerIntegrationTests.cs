...
public class PersonsControllerIntegrationTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task Index_DefaultRoute_ReturnsSuccessAndShowsPersons()
    {
        ...
        var rows = doc.DocumentNode.QuerySelectorAll("table tbody tr").ToList();
    ...
    public async Task Index_SortByNameASC_ReturnsSortedAscending()
    {
        ...
        var names = doc
            .DocumentNode.QuerySelectorAll("table tbody tr td:first-child")
    ...
    [Fact]
    public async Task Index_EmptyDb_ShowsEmptyStateWithNoRows()
    {
        var response = await _client.GetAsync("/Persons/Index");
        response.IsSuccessStatusCode.Should().BeTrue();
        ...
        doc.DocumentNode.QuerySelectorAll("table tbody tr").Should().BeEmpty();
        doc.DocumentNode.QuerySelector(".empty-state").Should().NotBeNull();
    }
    ...
    public async Task Edit_Get_ReturnsViewWithPersonData()
    {
        ...
        doc.DocumentNode.QuerySelector("input[name='Id']")
            ?.GetAttributeValue("value", "")
            .Should()
            .Be(person.Id.ToString());
        doc.DocumentNode.QuerySelector("input[name='Name']")
            ?.GetAttributeValue("value", "")
            .Should()
            .Be("Sam");
        doc.DocumentNode.QuerySelector("input[name='Email']")
            ?.GetAttributeValue("value", "")
            .Should()
            .Be("sam@test.com");
        doc.DocumentNode.QuerySelector("textarea[name='Address']")
            ?.InnerText.Trim()
            .Should()
            .Be("Addr");
    }
    ...
    public async Task Edit_Post_Invalid_ReturnsViewWithErrors()
    {
        ...
        doc.DocumentNode.QuerySelectorAll(".validation-error").Should().HaveCountGreaterThan(0);
        doc.DocumentNode.QuerySelector("select[name='CountryId']")
            .QuerySelectorAll("option")
            .Select(o => o.GetAttributeValue("value", "").Trim())
            .Should()
            .Contain(country.Id.ToString());
    }
    ...
    [Fact(
        Skip = "InMemory provider does not support ExecuteDeleteAsync used by repository.DeleteAsync; skip integration delete POST"
    )]
    ...
    public async Task Create_Post_InvalidRequest_ReturnsViewWithErrors()
    {
        ...
        var errors = doc.DocumentNode.QuerySelectorAll(".validation-error").ToList();
